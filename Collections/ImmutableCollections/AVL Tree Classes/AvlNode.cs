...
namespace Sanford.Collections.ImmutableCollections
{
	...
	internal class AvlNode : IAvlNode
	{
		...
		private readonly IAvlNode m_leftChild;
		private readonly IAvlNode m_rightChild;
		...
		public AvlNode(object data, IAvlNode leftChild, IAvlNode rightChild)
		{
			...
			m_count = 1 + leftChild.Count + rightChild.Count;
			m_height = 1 + Math.Max(leftChild.Height, rightChild.Height);
		}
		...
		private static IAvlNode DoLLRotation(IAvlNode node)
		{
			/*
		...
		private static IAvlNode DoLRRotation(IAvlNode node)
		{
			/*
		...
		private static IAvlNode DoRRRotation(IAvlNode node)
		{
			/*
		...
		private static IAvlNode DoRLRotation(IAvlNode node)
		{
			/*
		...
		{
			get { return m_rightChild.Height - m_leftChild.Height; }
		}
